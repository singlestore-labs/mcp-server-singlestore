...
class SingleStoreOAuthProvider(
    ...
    async def authorize(
        ...
        if (
            ...
            and str(params.redirect_uri) not in client.redirect_uris
        ):
            raise AuthorizeError(
                error="invalid_request",
    ...
    async def exchange_authorization_code(
        self,
        client: OAuthClientInformationFull,
        authorization_code: SingleStoreAuthorizationCode,
    ) -> OAuthToken:
        """
    ...
    async def revoke_token(
        ...
        if isinstance(token, SingleStoreAccessToken):
            ...
            for rt_key, rt in list(refresh_tokens.items()):
                if rt.client_id == token.client_id and set(rt.scopes) == set(
                    token.scopes
                ):
            ...
            for at_key, at in list(access_tokens.items()):
                if at.client_id == token.client_id and set(at.scopes) == set(
                    token.scopes
                ):
